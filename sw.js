const CACHE="scene-board-v6-10-5";
...
self.addEventListener("activate",e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("scene-board-")&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
